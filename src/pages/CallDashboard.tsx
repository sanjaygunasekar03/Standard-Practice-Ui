...
const BatchColumn: React.FC<BatchColumnProps> = ({ title, batches, showViewAll = false, count, showToday = false }) => {
  return (
    <div className="flex-1 bg-gray-50 rounded-lg">
      <div className="text-white p-4 rounded-t-lg" style={{ backgroundColor: '#0f1222' }}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {showToday && (
              <div className="flex items-center space-x-1">
                ...
                <span className="px-2 py-1 bg-gray-700 rounded text-xs">Today</span>
                <button className="text-white hover:text-gray-300">›</button>
              </div>
            )}
            {showViewAll && (
              <span className="text-sm text-gray-300">View all ({count || 0})</span>
            )}
            {title === 'Completed' && (
